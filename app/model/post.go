...
const stmtNumberOfPosts = "SELECT count(*) FROM posts WHERE %s AND is_deleted=false"

const stmtAuthorNumberOfPosts = "SELECT count(*) FROM posts WHERE created_by=(SELECT id FROM users WHERE name = ?) AND %s"

const stmtGetAllPostList = `SELECT * FROM posts WHERE %s ORDER BY %s AND is_deleted=false`
const stmtGetPostList = `SELECT * FROM posts WHERE %s AND is_deleted=false ORDER BY %s LIMIT ? OFFSET ?`

const stmtGetPostAuthor = `SELECT * FROM posts WHERE created_by=(SELECT id FROM users WHERE name = ?) AND %s AND is_deleted=false ORDER BY %s LIMIT ? OFFSET ?`

const stmtDeletePostById = `UPDATE posts SET is_deleted=true WHERE id = ?`
...
type Post struct {
	...
	Abstract        string     `meddler:"abstract",json:"abstract"`
	...
	Is_deleted      bool       `meddler:"is_deleted"`
}
...
func (p *Post) Excerpt() string {
	return utils.Html2Excerpt(p.Html, 55)
}
...
func (p *Post) Save(tags ...*Tag) error {
	...
	if p.Slug == "" {
		return fmt.Errorf("Title can not be empty or root")
	}
...
func (p *Post) UpdateFromRequest(r *http.Request) {
	...
	p.Abstract = r.FormValue("abstract")
...
// GetAuthorNumberOfPosts gets the total number of posts in the DB.
func GetAuthorNumberOfPosts(isPage bool, published bool, name string) (int64, error) {
	var count int64
	var where string
	if isPage {
		where = `page = 1`
	...
	row = db.QueryRow(fmt.Sprintf(stmtAuthorNumberOfPosts, where), name)
	err := row.Scan(&count)
	if err != nil {
		return 0, err
	}
...
func (posts *Posts) GetPostAuthor(name string, page, size int64, isPage bool, onlyPublished bool, orderBy string) (*utils.Pager, error) {
	...
	err = meddler.QueryAll(db, posts, fmt.Sprintf(stmtGetPostAuthor, where, safeOrderBy), name, size, pager.Begin)
	return pager, err
}
