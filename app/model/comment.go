...
// GetAuthorNumberOfComments returns the total number of comments in the DB.
func GetAuthorNumberOfComments(name string) (int64, error) {
	var count int64
	var row *sql.Row
	row = db.QueryRow(stmtGetAuthorCommentCount, name)
	err := row.Scan(&count)
	if err != nil {
		return 0, err
	}
...
// GetCommentList returns a new pager based on the total number of comments.
func (c *Comments) GetAuthorCommentList(name string, page, size int64, onlyApproved bool) (*utils.Pager, error) {
	var pager *utils.Pager

	count, err := GetAuthorNumberOfComments(name)
	pager = utils.NewPager(page, size, count)
	...
	err = meddler.QueryAll(db, c, fmt.Sprintf(stmtAuthorGetCommentList, where), name, size, pager.Begin)
	return pager, err
}
...
const stmtGetAllCommentCount = `SELECT count(*) FROM comments WHERE is_deleted=false`
const stmtGetAuthorCommentCount = `select count(*) from comments where post_id=any(select id from posts where created_by=(select id from users where name=?)) and is_deleted=false`

const stmtDeleteCommentById = `UPDATE comments SET is_deleted = true WHERE id = ?`

const stmtGetCommentList = `SELECT * FROM comments WHERE is_deleted=false %s ORDER BY created_at DESC LIMIT ? OFFSET ?`
const stmtAuthorGetCommentList = `SELECT * FROM comments WHERE post_id=any(SELECT id FROM posts WHERE created_by=(SELECT id FROM users WHERE name = ? )) %s AND is_deleted=false ORDER BY created_at DESC LIMIT ? OFFSET ?`
