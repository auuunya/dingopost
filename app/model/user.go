...
const stmtGetAllUser = `SELECT * FROM users WHERE %s AND is_deleted=false ORDER BY %s LIMIT ? OFFSET ?`
...
type User struct {
	...
	IsPage         bool       `meddler:"page",json:"is_page"`
	...
	Role           bool       `meddler:"role"` //1 = Administrator, 2 = Editor, 3 = Author, 4 = Owner
...
func NewUser(email, name string, role bool) *User {
	return &User{
		Email:     email,
		Name:      name,
		Role:      role,
...
// GetAllUser
func (u *Users) GetAllUser(page, size int64, isPage bool, orderBy string) (*utils.Pager, error) {
	...
	err = meddler.QueryAll(db, u, fmt.Sprintf(stmtGetAllUser, where, orderBy), size, pager.Begin)
	return pager, err
}
