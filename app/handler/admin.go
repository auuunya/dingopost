...
func AdminHandler(ctx *golf.Context) {
	...
	if u.Role == true {
		...
		ctx.Loader("admin").Render("home.html", map[string]interface{}{
			"Title":    "仪表盘",
			"Statis":   model.NewStatis(ctx.App, u),
			"User":     u,
			"Messages": m,
...
func ProfileHandler(ctx *golf.Context) {
	...
	pager, err := users.GetAllUser(int64(page), 10, false, "created_at DESC")
	if err != nil {
		panic(err)
	}
	if user.Role == true {
		ctx.Loader("admin").Render("users.html", map[string]interface{}{
			"Title": "用户",
			"Pager": pager,
			"Users": users,
			"User":  user,
...
func PostSaveHandler(ctx *golf.Context) {
	...
	p.Id = int64(idInt)
	p.Abstract = ctx.Request.FormValue("abstract")
...
func AdminPostHandler(ctx *golf.Context) {
	...
	if u.Role == true {
		...
		pager, err = posts.GetPostAuthor(u.Name, int64(page), 10, false, false, "created_at DESC")
	}
	if err != nil {
		panic(err)
	}
...
func CommentViewHandler(ctx *golf.Context) {
	...
	if u.Role == true {
		...
		pager, err = comments.GetAuthorCommentList(u.Name, int64(page), 10, false)
	}
	if err != nil {
		panic(err)
	}
