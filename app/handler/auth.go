...
func getRoleName() *Config {
	config := Config{}
	path := model.AppPath(ConfigName)
...
func AuthSignUpHandler(ctx *golf.Context) {
	...
	config := getRoleName()
	if name == config.RoleName {
		err = model.NewUser(email, name, true).Create(password)
		if err != nil {
			ctx.Abort(500)
			return
		}
	} else {
		err = model.NewUser(email, name, false).Create(password)
		if err != nil {
			ctx.Abort(500)
			return
		}
